...
public class GridManager : MonoBehaviour
{
    ...
    public int emptySpaceFrequency = 7; // Esim. 7 --> sein‰ tulee ruutuun 1/7 tod.n‰k.
    ...
    void GenerateGrid()
    {
        ...
        for (int i = 0; i < columns; i++)
        {
            for (int j = 0; j < columns; j++)
            {
                ...
                {
                    int rnd = UnityEngine.Random.Range(0, emptySpaceFrequency); // mill‰ mahiksella tulee sein‰
                    grid[i, j] = rnd == 1 ? 1 : 0;
                }
